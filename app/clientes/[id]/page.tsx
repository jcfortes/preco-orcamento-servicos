    ...
    Promise.all([
      ...
      if (cliente) {
        setForm({
          ...
          cnpj: cliente.cnpj || "", inscricao_municipal: cliente.inscricao_municipal || "",
          segmento_id: cliente.segmento_id || "", regime_tributario: cliente.regime_tributario || "",
          porte: cliente.porte || "", origem: cliente.origem || "",
        });
        if (cliente.segmento_id && segs) {
          const seg = segs.find((s) => s.id === cliente.segmento_id);
          if (seg?.pai_id) setSegmentoPaiId(seg.pai_id);
        }
      }
      setSegmentos(segs || []);
      setLoading(false);
